const String CACHED_TOKEN = 'CACHED_TOKEN';

//const String API_BASE_URL = 'https://intranet.proonix.com.pe'; // add the base url here
//const String API_BASE_URL = 'http://192.168.185.32/soal/'; // add the base url here
const String API_BASE_URL = 'http://192.168.43.29/soal/'; // add the base url here
const String LOGIN_USER = 'tokens';
const String CREATE_USER = 'create';

const String LOGGING_ERROR = 'Could not login successfully, please check your email and password';
const String LOGGING_OUT_ERROR = 'Could not logout successfully, please try again later';
const String NO_CONNECTION_ERROR = 'You are not connected to the Internet';
const String CHANGE_PASSWORD_ERROR = 'Could not change the password. Please try again later';

const double DEFAULT_PAGE_PADDING = 20;

//routes
const String HOME_ROUTE = '/';
const String LOGIN_ROUTE = '/login';
const String SPLASH_ROUTE = '/splash';
const String HOME_GERENCIA = '/gerencia';
const String PAGE_MENU = '/menu';
