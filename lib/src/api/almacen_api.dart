...
class AlmacenApi {
  ...
  Future<ApiResultModel> getStockRecursoAlmacenes() async {
    ApiResultModel result = ApiResultModel();
    try {
      final url = '$API_BASE_URL/api/Almacen/ws_listar_stock_almacen';
      String? token = await StorageManager.readData('token');
      final response = await http.post(Uri.parse(url), body: {
        'app': 'true',
        'tn': token,
      ...
      if (response.statusCode == 200) {
        await almacenDatabase.deleteAll();

        for (var i = 0; i < decodedData['result']['datos'].length; i++) {
          SedesModel sedesModel = SedesModel();
          sedesModel.idSede = decodedData['result']['datos'][i]['id_sede'];
          sedesModel.sedeNombre = decodedData['result']['datos'][i]['sede_nombre'];
          await sedesDatabase.insertarSedes(sedesModel);

          var recursos = decodedData['result']['datos'][i]['informacion'];
